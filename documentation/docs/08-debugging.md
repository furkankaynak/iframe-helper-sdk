...
| `CONFIG_UNBOUNDED_RESIZE`          | warn  | Resize plugin is enabled without max bounds for every active axis.   | Add `maxWidthPx` and/or `maxHeightPx`, or use `securityProfile: 'strict'` to fail fast. |
