...
| `CONFIG_INVALID_RESIZE`            | `resizePlugin()` axis is invalid, a resize bound or offset is invalid, `onResize` is not a function, or a minimum bound is greater than its matching maximum.                                       | Use a valid axis, finite integer offsets, non-negative integer bounds, and minimum values that do not exceed maximum values. |
