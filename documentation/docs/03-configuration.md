...
Plugins are optional parent-side extensions passed through the second argument to `createIframeBridge()` or `createTypedIframeBridge()`. They are not part of `IframeBridgeConfig`; this keeps optional runtime code tree-shakable and outside the core package entry point.
...
`resizePlugin()` is disabled unless registered. Calling `resizePlugin()` or `resizePlugin({})` enables both-axis resizing with no bounds. In `securityProfile: 'development'`, unbounded active axes emit `CONFIG_UNBOUNDED_RESIZE` through diagnostics. In `securityProfile: 'strict'`, every active axis must have a matching max bound or bridge creation throws `CONFIG_INVALID_RESIZE`.

For full plugin behavior, event claiming rules, and resize options, see [Plugins](./plugins) and [Resize Plugin](./plugins/resize).
...
- **[Plugins](./plugins)** — Register optional tree-shakable parent-side bridge extensions.
- **[Resize Plugin](./plugins/resize)** — Let the iframe request bounded parent-applied dimensions.
